...
function submitDriverData(event) {
  ...
  document.getElementById('confirmName').textContent = driverName;
  document.getElementById('confirmInvoice').textContent = invoiceNumber;
  document.getElementById('confirmTime').textContent = timeReceived;
  document.getElementById('confirmLocation').textContent = location;

  // Toggle UI
  document.getElementById('driverForm').classList.add('hidden');
  document.getElementById('confirmationMode').classList.remove('hidden');
  document.getElementById('formHeader').innerHTML = '<i data-lucide="clipboard-check" style="width:20px;height:20px;color:#0F2341;"></i> Confirm Delivery Info';
